...
public class QuotaManagerTest {
    ...
    @Before
    public void init() {
        quotaManager = new QuotaManager(actionExecutor, false);
    }
    ...
    public void test_shouldThrowExceptionIfResultIsNull() {
        ...
        QuotaExistsInCurrentTenantRequest quotaRequest = QuotaExistsInCurrentTenantRequest.builder()
                .withTenantName("tenantName")
                .build();

        M3Result<Boolean> rawResult = M3Result.success("id", true);
        Mockito.when(actionExecutor.executeAction(eq(principal), eq(M3SdkVersion.V3), any())).thenReturn(rawResult);
        M3Result<Boolean> result = quotaManager.existsInCurrentTenant(principal, quotaRequest);
